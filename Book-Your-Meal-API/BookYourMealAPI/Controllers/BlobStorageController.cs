        ...
        public async Task<IActionResult> InsertFile(IFormFile asset)
        {
            try
            {
                if (CloudStorageAccount.TryParse(config.Value.StorageConnection, out CloudStorageAccount storageAccount))
                {
                    ...
                    Microsoft.WindowsAzure.Storage.StorageUri url = blockBlob.SnapshotQualifiedStorageUri;
                    string imageUrl = url.PrimaryUri.AbsoluteUri;
                ...
                {
                    return BadRequest(new { url="Error has occured"});
                }
            }
            catch (Exception ex)
            {
                return BadRequest(new { url=ex.Message});
            }
        }
